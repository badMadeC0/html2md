...
$ConvertBtn.Add_Click({
    ...
    if ($urlList.Count -gt 1) {
        ...
        if (Test-Path -LiteralPath $venvExe) {
            ...
            $psi.Arguments = "-NoExit -Command `"& '$safeVenvExe' --url '$safeUrl' --outdir '$safeOutDir' --all-formats`""
        }
        elseif (Test-Path -LiteralPath $pyScript) {
            $LogBox.AppendText("Found Python script: $pyScript`r`n")
            $psi.Arguments = "-NoExit -Command `"& $pyCmd '$safePyScript' --url '$safeUrl' --outdir '$safeOutDir' --all-formats`""
        }
        else {
