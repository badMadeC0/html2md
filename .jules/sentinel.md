...
**Learning:** `csv.DictWriter` does not automatically sanitize fields for formula injection. Also, spreadsheets may strip leading whitespace before detecting formulas, so `    =SUM(...)` is also dangerous.
**Prevention:** Prefix any field value where `lstrip()` starts with `=`, `+`, `-`, or `@` with a single quote `'` to force it to be treated as text by spreadsheet applications.
