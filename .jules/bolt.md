...
## 2024-08-12 - CSV Injection Sanitization Performance
**Learning:** Checking for whitespace using `str.isspace()` before calling expensive string methods like `lstrip()` and `startswith()` can yield significant micro-performance gains (e.g. ~2x faster in local benchmarks) in hot loops where most strings do not start with whitespace.
**Action:** Always check fast-paths and simple character predicates (`isspace`, `startswith` with tuple) before running more allocation-heavy or iterative string methods like `lstrip` in frequent operations like CSV writing/sanitization.
