...
## 2024-03-26 - CSV Formula Sanitization Performance
**Learning:** `str.lstrip().startswith(...)` creates a new string object each time it is called. In the hot path of processing hundreds of thousands of cells for CSV export, this string allocation and scanning becomes a noticeable bottleneck.
**Action:** Replace `lstrip().startswith()` with character-by-character checks `value[0]` and only use `lstrip()` when the leading character is confirmed to be whitespace. Ensure `None` values map efficiently without allocating default strings in loops (e.g., avoid `rec.get(key, "")` when the downstream sanitization function already maps `None` to `""`).
