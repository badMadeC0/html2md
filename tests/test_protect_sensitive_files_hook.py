...
def test_registered_hook_command_blocks_sensitive_file_from_path_with_spaces(
    ...
    command_template = settings["hooks"]["PreToolUse"][0]["hooks"][0]["command"]

    # Create a real directory (not a symlink) with spaces in its name so the
    # test is reliable on Windows runners that restrict symlink creation.
    project_dir_with_spaces = tmp_path / "project path with spaces"
    hooks_dir = project_dir_with_spaces / ".claude" / "hooks"
    hooks_dir.mkdir(parents=True)
    shutil.copy2(str(hook_src), str(hooks_dir / hook_src.name))

    # Expand the command template in Python to avoid platform shell differences
    # (cmd.exe does not expand $VAR syntax, so shell=True is not portable).
    static_suffix = command_template.replace('"$CLAUDE_PROJECT_DIR"', "").lstrip("/")
    hook_path = project_dir_with_spaces / static_suffix.replace("/", os.sep)
    payload = json.dumps(
