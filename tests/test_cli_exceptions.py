                ...
                with patch('markdownify.markdownify', return_value="# Hello"):
                    with patch('os.path.exists', return_value=True):
                        ...
                        with patch('builtins.open', side_effect=custom_open) as mock_open:
                            ...
                            mock_open_calls = [c for c in mock_open.call_args_list if not (c.args and ('locale' in str(c.args[0]) or 'gettext' in str(c.args[0]) or 'LC_MESSAGES' in str(c.args[0])))]
