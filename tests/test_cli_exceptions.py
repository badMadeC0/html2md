                    ...
                    with patch("os.path.exists", return_value=True):
                        ...
                        def mock_open_fn(*args, **kwargs):
                            if (
                                ...
                                and args[0].endswith(".md")
                            ):
                                return unittest.mock.mock_open()(*args, **kwargs)
                            return original_open(*args, **kwargs)

                        with patch(
                            "builtins.open", side_effect=mock_open_fn
                        ) as mock_open:
                            ...
                            # Only assert that mock_open was not called with our target file
                            for call in mock_open.call_args_list:
                                if len(call[0]) > 0 and isinstance(call[0][0], str):
                                    self.assertFalse(call[0][0].endswith(".md"))
