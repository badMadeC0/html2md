                    ...
                    with patch("os.path.exists", return_value=True):
                        with patch("builtins.open") as mock_open:
                            ...
                            with patch("os.path.realpath", side_effect=fake_realpath):
                                # Avoid gettext triggering mock builtins.open by skipping gettext or ensuring open works
                                with patch(
                                    "argparse._", lambda x: x
                                ):  # simple hack to avoid gettext translation in mock
                                    main(
                                        [
                                            "--url",
                                            "http://example.com/a",
