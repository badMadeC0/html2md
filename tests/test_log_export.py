...
def test_log_export_sanitizes_tab_and_cr(tmp_path):
    """Test values starting with tab or carriage return are escaped."""
    input_file = tmp_path / "tab_cr.jsonl"
    output_file = tmp_path / "tab_cr.csv"

    with open(input_file, "w", encoding="utf-8") as f:
        f.write('{"tab": "\\tPlain text", "cr": "\\rPlain text"}\n')

    argv = ["--in", str(input_file), "--out", str(output_file), "--fields", "tab,cr"]
    main(argv)
    ...
    with open(output_file, "r", encoding="utf-8", newline="") as f:
        ...
        rows = list(reader)
        assert len(rows) == 1
        assert rows[0]["tab"] == "'\tPlain text"
        assert rows[0]["cr"] == "'\rPlain text"
