...
# Fastest lookup with tuple and short-circuiting logic
def _sanitize_formula_new(value: str) -> str:
    if not value or value[0] == "'":
        return value

    # Check first character against dangerous prefixes directly
    if value[0] in ("=", "+", "-", "@"):
        return f"'{value}"

    # Only perform expensive lstrip() if there's leading whitespace
    if value[0].isspace():
