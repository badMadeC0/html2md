...
def process_url(
    target_url: str, session, md, outdir_path: Path | None, real_outdir: Path | None
) -> int:
...
def main(argv=None):
    ...
    if args.url or args.batch:
        ...
        if args.url:
            code = process_url(args.url, session, md, outdir_path, real_outdir)
            if code:
                exit_code = code
        ...
        if args.batch:
            ...
            with open(args.batch, "r", encoding="utf-8") as f:
                for line in f:
                    u = line.strip()
                    if u:
                        code = process_url(u, session, md, outdir_path, real_outdir)
                        exit_code |= code
