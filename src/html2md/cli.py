...
def _process_url(target_url: str, session, md_func, outdir: str | None = None) -> None:
...
def main(argv=None):
    ...
    if args.url or args.batch:
        try:
            import requests  # type: ignore  # pylint: disable=import-outside-toplevel, unused-import
        ...
        if args.url:
            _process_url(args.url, session, md, args.outdir)
        ...
        if args.batch:
            ...
            with open(args.batch, "r", encoding="utf-8") as f:
                for line in f:
                    u = line.strip()
                    if u:
                        _process_url(u, session, md, args.outdir)
