...
def is_safe_url(url: str) -> bool:
    """Validate that a URL resolves to a safe, public IP address to prevent SSRF."""
    parsed = urlparse(url)
    ...
    try:
        ...
        for res in addr_info:
            ...
            # Check if the IP is public (not private, loopback, link-local, multicast, or reserved)
            if (
                ip_obj.is_private
    ...
    if args.url or args.batch:
        ...
        def process_url(target_url: str) -> int:
            ...
            if not is_safe_url(target_url):
                print(
                    f"Error: URL '{target_url}' resolves to a private or reserved IP address. "
                    "For security reasons (SSRF protection), only public IP addresses are allowed.",
                    file=sys.stderr,
                )
                return 1
