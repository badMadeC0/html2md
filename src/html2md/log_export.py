...
def main(argv=None):
    ...
    is_simple_mapping = all(i == o for i, o in mapping)
    ...
    with inp.open("r", encoding="utf-8") as fi, out.open(
        ...
        for line in fi:
            ...
            if is_simple_mapping:
                # Fast path: mutate directly if fields match exactly, letting `extrasaction='ignore'` handle the rest
                for _, output_name in mapping:
                    if output_name in rec:
                        rec[output_name] = _sanitize_value(rec[output_name])
                    else:
                        rec[output_name] = ""
                w.writerow(rec)
            else:
                ...
                for input_name, output_name in mapping:
                    if input_name in rec:
                        row[output_name] = _sanitize_value(rec[input_name])
                    else:
                        row[output_name] = ""
