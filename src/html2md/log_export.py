...
def _sanitize_formula(value: str) -> str:
    ...
    if not value or value[0] == "'":
        return value
    # Optimization: Use string for single char 'in' check, it's faster than tuple
    if value[0] in "=+-@" or value.lstrip().startswith(_DANGEROUS_PREFIXES):
        return f"'{value}"
    return value
...
def _sanitize_value(value: object) -> object:
    ...
    if isinstance(value, str):
        # Optimization: Inline _sanitize_formula to avoid function call overhead
        if not value or value[0] == "'":
            return value
        if value[0] in "=+-@" or value.lstrip().startswith(_DANGEROUS_PREFIXES):
            return f"'{value}"
