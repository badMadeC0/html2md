...
def _sanitize_formula(value: str) -> str:
    ...
    Optimized: Avoids allocating a new string via lstrip(), which
    improves performance significantly for large inputs or when
    called millions of times during CSV export.
    """
    if value.startswith("'"):
        return value
