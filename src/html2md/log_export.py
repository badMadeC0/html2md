...
def main(argv=None):
    ...
    # Pre-extract input names to avoid tuple unpacking in the inner loop
    input_names = [input_name for input_name, _ in mapping]

    with inp.open("r", encoding="utf-8") as fi, out.open(
        "w", newline="", encoding="utf-8"
    ) as fo:
        # Use csv.writer instead of DictWriter to eliminate per-row dictionary allocation
        # and DictWriter mapping overhead.
        w = csv.writer(fo)
        ...
        for line in fi:
            ...
            # Directly generate list of values avoiding DictWriter overhead
            row = [_sanitize_value(rec.get(k, "")) for k in input_names]
            w.writerow(row)
