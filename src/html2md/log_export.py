...
def _sanitize_formula(value: str) -> str:
    ...
    if c in _DANGEROUS_PREFIXES:
        return f"'{value}"
    # Optimization: If the first char is not whitespace, we don't need to lstrip()
    # This avoids creating a new string copy for the vast majority of regular strings.
    if not c.isspace():
        return value
