...
def _sanitize_formula(value: str) -> str:
    ...
    # Optimization: avoid lstrip() memory allocation for non-whitespace prefixes.
    # Only perform the expensive lstrip().startswith() if the first char is whitespace.
    c = value[0]
...
def main(argv=None):
    ...
    out = Path(args.out)
    with (
        inp.open("r", encoding="utf-8") as fi,
